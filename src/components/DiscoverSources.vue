...
export default {
  ...
  computed: {
    sourcesToFollow: function() {
      ...
      return this.sourceResults.filter(source => (!this.followedSourcesIds.includes(source.id)
        && source.id != auth_user_id));
    },
    followedSourcesIds: function() {
      return this.followed_sources.map(source => source.id);
    },
